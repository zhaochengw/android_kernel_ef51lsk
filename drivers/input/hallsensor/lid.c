...
static unsigned int hall_sensor_gpio = 34;
...
//	int gpio = irq_to_gpio(irq);
	int gpio = MSM_GPIO_TO_INT(hall_sensor_gpio);
	if (gpio == irq)
	{
...
static int lid_irq_hall_sensor(void)
{
	...
	unsigned irq = MSM_GPIO_TO_INT(hall_sensor_gpio);	
	...
	// GPIO intialize
	gpio_tlmm_config(GPIO_CFG(gpio, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
...
//		input_report_key(lid_indev, KEY_MR_POUCH_OPEN, 1);
//		input_report_key(lid_indev, KEY_MR_POUCH_OPEN, 0);
	}
	else
	{
		input_report_switch(lid_indev, SW_LID, 1);
//		input_report_key(lid_indev, KEY_MR_POUCH_CLOSE, 1);
//		input_report_key(lid_indev, KEY_MR_POUCH_CLOSE, 0);
	}
	input_sync(lid_indev);
	LID_NOTICE("KEY_MR_POUCH report value = %d\n", value);
}
...
//	set_bit(KEY_MR_POUCH_CLOSE, lid_indev->keybit);
//	set_bit(KEY_MR_POUCH_OPEN, lid_indev->keybit);
