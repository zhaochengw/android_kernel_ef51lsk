				...
				if(value >= 0){
					...
					if(p_pantech_ext_config_desc){
                    	...
                    	printk(KERN_INFO "cdrom: %d interface: %d \n",
                    			pantech_cdrom_enabled, pantech_ext_config_desc_list[0].desc.function[0].bFirstInterfaceNumber);
                    		if(pantech_cdrom_enabled)
                                pantech_ext_config_desc_list[0].desc.function[0].bFirstInterfaceNumber = 1;
#endif                       
			    ...
			    const char *buff, size_t size)
{
	...
	if (enabled && !dev->enabled) {
...
#ifdef CONFIG_PANTECH_ANDROID_MTP
		if((function_is_enabled(dev , "mtp") || function_is_enabled(dev , "ptp")) && function_is_enabled(dev , "adb")){
			//printk("^^^^^ mtp_adb_is_enabled :%d\n",value);	
			device_desc.bDeviceClass = 0xEF;
			device_desc.bDeviceSubClass = 0x02;
			device_desc.bDeviceProtocol = 0x01;
		}else if(function_is_enabled(dev , "mtp") ){
			printk("^^^^^ mtp only enabled\n");	
			device_desc.bDeviceClass = 0xFF;
			device_desc.bDeviceSubClass = 0xFF;
			device_desc.bDeviceProtocol = 0x00;
		}else if(function_is_enabled(dev , "ptp") ){
			printk("^^^^^ ptp only enabled\n");	
			device_desc.bDeviceClass = 0x06;
			device_desc.bDeviceSubClass = 0x01;
			device_desc.bDeviceProtocol = 0x01;
		}else{
			if(device_desc.idVendor == 0x05C6){
				dev_dbg(pdev, "^^^^^ enable_store : It's Qualcomm\n");	
				device_desc.bDeviceClass = 0x00;
				device_desc.bDeviceSubClass = 0x00;
				device_desc.bDeviceProtocol = 0x00;
			}else{
				dev_dbg(pdev, "^^^^^ enable_store : It's SKY\n");	
				device_desc.bDeviceClass = 0x02;
				device_desc.bDeviceSubClass = 0x00;
				device_desc.bDeviceProtocol = 0x00;
			}
		...
		if((function_is_enabled(dev , "mtp")) && (function_is_enabled(dev , "modem"))){
			printk("^^^^^ LGU PC data mode\n");	
			device_desc.bDeviceClass = 0x02;
			device_desc.bDeviceSubClass = 0x00;
			device_desc.bDeviceProtocol = 0x00;
		}
...
{
	return snprintf(buf, PAGE_SIZE, "[[p13120]] usb_mdm_control_show :: is_mdm_usb_control_enabled = %d\n", is_mdm_usb_control_enabled);
}
...
static ssize_t usb_mdm_control_store(struct device *pdev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	struct android_dev *dev = dev_get_drvdata(pdev);
//	char *mdm_control_connected[2] = { "USB_STATE=CONNECTED", NULL };
	...
	if(is_mdm_usb_control_enabled){
		printk("[[p13120]] usb_mdm_control_store : disconnect!\n");
	kobject_uevent_env(&dev->dev->kobj, KOBJ_CHANGE, mdm_control_disconnected);

	}else{
		printk("[[p13120]] usb_mdm_control_store : connect!\n");
//		kobject_uevent_env(&dev->dev->kobj, KOBJ_CHANGE, mdm_control_connected);
			}
...
#ifdef FEATURE_MDM_APK_UMS_CTL
//static DEVICE_ATTR(usb_mdm_control, S_IRUGO | S_IWUGO, usb_mdm_control_show, usb_mdm_control_store);
