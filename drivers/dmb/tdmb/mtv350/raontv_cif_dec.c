										...
										UINT pkt_offset, UINT *invalid_offset)
{
	...
	U8 fic_buf[3];
	const U8 *fic_ptr;
	UINT org_fic_size = hdr->fic_size;
#if (RTV_NUM_DAB_AVD_SERVICE >= 2)
	UINT i;
#endif
	...
	if (hdr->fic_size) {
		...
		if ((fic_ptr[0] != 0x05) || (fic_ptr[1] != 0x00) || (fic_ptr[2] != 0xE0)) {
			hdr->fic_size = 0;
			g_nNumFicSizeNoData++;
			CIF_DBGMSG3("8888888 proc_truncated_packet 1st(0x%02X) 2nd(0x%02X) 3rd(0x%02X)\n", fic_ptr[0], fic_ptr[1], fic_ptr[2]);
		}
		else {
							...
							UINT *invalid_offset)
{
	...
	if ((cifb->prev_len + cifb->curr_len) >= (hdr->pkt_size + RTV_CIF_HEADER_SIZE)) {
		...
		if (status == PARSER_STATUS__OK) {
			...
			if (tsi_ret == TRUE) {
				*invalid_offset = 0;
				pkt_offset = hdr->pkt_size;
				return proc_truncated_packet(hdr, next_hdr, cifb, pkt_offset, invalid_offset);
			}
		}
...
unsigned long prev_ic_jiffies;

int rtvCIFDEC_Decode(RTV_CIF_DEC_INFO *ptDecInfo, UINT nMscBufSize,
...
#ifdef RTV_REMOVE_NOISE_MSC
	if (g_fRtvCerMscSet == FALSE){
	  unsigned long diff_jiffies = get_jiffies_64() - prev_ic_jiffies;
	  if (jiffies_to_msecs(diff_jiffies) >= 3000) {
    	  //CIF_DBGMSG0("[rtvCIFDEC_Decode] $$$$$$$$$$$$ g_fRtvCerFicSet\n");
    	  reset_processing_data();
    	  return 1;
    	 }
	}
...
void rtvCIFDEC_Init(void)
{
	...
	prev_ic_jiffies = get_jiffies_64();
#else
