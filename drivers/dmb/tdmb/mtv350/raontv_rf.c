...
#ifdef RTV_FM_ENABLE
		case RTV_TV_MODE_FM :   REG54 = 0x1C; break;
#endif

#ifdef RTV_TDMB_ENABLE
		case RTV_TV_MODE_TDMB : REG54 = 0x1C; break;
#endif
...
INT rtvRF_ConfigureAdcClock(E_RTV_TV_MODE_TYPE eTvMode, E_RTV_ADC_CLK_FREQ_TYPE eAdcClkFreqType)
{
	...
	if(i == 10)
	{
		RTV_DBGMSG0("[rtvRF_ConfigureAdcClock] ADC clock unlocked!Check the power supply.\n");
...
#if defined(RTV_TDMB_ENABLE) && (RTV_SRC_CLK_FREQ_KHz == 19200)
	RTV_REG_MAP_SEL(HOST_PAGE);
	RTV_REG_SET(0x1A, 0x87);
	RTV_REG_SET(0x18, 0xC0);
...
{	
	...
	if(i == 10)
	{
		RTV_DBGMSG0("[rtvRF_ChangeAdcClock] ADC clock unlocked!Check the power supply.\n");
		return RTV_ADC_CLK_UNLOCKED;
	}
...
INT rtvRF_SetFrequency(E_RTV_TV_MODE_TYPE eTvMode, UINT nChNum, U32 dwChFreqKHz)
{
	...
	switch( eTvMode )
	{
		...
		case RTV_TV_MODE_DAB_L :
			...
			break;
	#endif /* #ifdef RTV_DAB_LBAND_ENABLED */
#endif
	...
	switch( eTvMode )
	{
		...
		case RTV_TV_MODE_1SEG :			
			...
			RTV_REG_SET(0x3A, (lnacaphg_nIdx << 4 )| 0x07); 	
	#endif			
			...
			do{
				...
				rd_dwPllNF_Verify_val = (RTV_REG_GET(0x12) << 22) + (RTV_REG_GET(0x13) << 14) +(RTV_REG_GET(0x14) << 6) +(RD15 >> 2);
		...
		case RTV_TV_MODE_FM :		
			...
			do{
				...
				{ 
					RTV_REG_SET(0x20, 0x00);	
					RTV_DELAY_MS(2);	//2ms Delay 
				//	RTV_DBGMSG3("[rtvRF_SetFrequency] FM PLL verify Re-Try!!!  PLLNF_I2C = 0x%0x, PLLNF_I2C/2 = 0x%0x,   RD_PLLNF = 0x%0x\n", dwPllNF & ~0x1, wr_dwPllNF_Verify_val, rd_dwPllNF_Verify_val & ~0x1);
				}
			}while(--PLL_Verify_cnt);
		...
		case RTV_TV_MODE_TDMB :	
			...
			do{
				...
				//RTV_DBGMSG3("[rtvRF_SetFrequency] TDMB PLL PLLNF_I2C = 0x%0x, PLLNF_I2C/2 = 0x%0x,   RD_PLLNF = 0x%0x\n", dwPllNF & ~0x1, wr_dwPllNF_Verify_val, rd_dwPllNF_Verify_val & ~0x1);
				...
				{ 
					RTV_REG_SET(0x20, 0x00);	
					RTV_DELAY_MS(2);	//2ms Delay 
					RTV_DBGMSG3("[rtvRF_SetFrequency] TDMB PLL verify Re-Try!!!  PLLNF_I2C = 0x%0x, PLLNF_I2C/2 = 0x%0x,   RD_PLLNF = 0x%0x\n", dwPllNF & ~0x1, wr_dwPllNF_Verify_val, rd_dwPllNF_Verify_val & ~0x1);
				}
			}while(--PLL_Verify_cnt);
		...
		case RTV_TV_MODE_TDMB:
			...
			do{
				...
				{ 
					RTV_REG_SET(0x20, 0x00);	
					RTV_DELAY_MS(2);	//2ms Delay 
				//	RTV_DBGMSG3("[rtvRF_SetFrequency] DAB PLL verify Re-Try!!!  PLLNF_I2C = 0x%0x, PLLNF_I2C/2 = 0x%0x,   RD_PLLNF = 0x%0x\n", dwPllNF & ~0x1, wr_dwPllNF_Verify_val, rd_dwPllNF_Verify_val & ~0x1);
				}
			}while(--PLL_Verify_cnt);
