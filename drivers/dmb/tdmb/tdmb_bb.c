...
static uint8 tdmb_play_started=0;
...
void set_tdmb_data_jiffies(void)
{
  ...
  TDMB_MSG_BB("-->TDMB first data [%d]\n",jiffies_to_msecs(tdmb_kernel_time[TDMB_TIME_FIRST_MSC]));
}
#endif
...
int tdmb_bb_subch_start (uint8 *Regs, uint32 data_rate)
{
  ...
  {
    TDMB_MSG_BB("TDMB change Set_freq [%d]  Set_CH [%d]\n",jiffies_to_msecs(tdmb_kernel_time[TDMB_TIME_SET_FREQ]),jiffies_to_msecs(tdmb_kernel_time[TDMB_TIME_SET_CH]));
  }
  else
  {
    TDMB_MSG_BB("TDMB enter BB_init [%d]  Set_freq [%d]  Set_CH [%d]\n",jiffies_to_msecs(tdmb_kernel_time[TDMB_TIME_BB_INIT]),jiffies_to_msecs(tdmb_kernel_time[TDMB_TIME_SET_FREQ]),
                jiffies_to_msecs(tdmb_kernel_time[TDMB_TIME_SET_CH]));
    tdmb_play_started = 1;
  }
  reset_tdmb_kernel_time();
#endif
