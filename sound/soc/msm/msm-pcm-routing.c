...
int get_dai_mm(void){
	...
	for(aud_session=0; aud_session<MSM_FRONTEND_DAI_MM_SIZE; aud_session++){
		if(fe_dai_map[aud_session][SESSION_TYPE_RX]>0){
			break;
		}
	}
...
int get_aud_non_lpa_session_id(void){
	...
	for(aud_session=0; aud_session<MSM_FRONTEND_DAI_MM_SIZE; aud_session++){
		if(fe_dai_map[aud_session][SESSION_TYPE_RX]>0){
			aud_session_id = fe_dai_map[aud_session][SESSION_TYPE_RX];
			break;
		}
	}
...
int get_aud_lpa_session_id(void){
	...
	if(lpa_audio_session_activate && (fe_dai_map[2/*MULTIMEDIA3*/][SESSION_TYPE_RX]>0)){
		aud_session_id = fe_dai_map[2/*MULTIMEDIA3*/][SESSION_TYPE_RX];		
	}
...
//	pr_err("\n@#@#@#@#%s: aud_session : %d, fe_dai_map[aud_session][SESSION_TYPE_RX] : %d\n", __func__, aud_session, fe_dai_map[aud_session][SESSION_TYPE_RX]);
	return fe_dai_map[aud_session][SESSION_TYPE_RX];
#endif
}
